...
function tes8(record) {
   ...
   let trans = [];
    ...
    for(var i=0; i<trans.length; i++){
        if(trans[i].length === 3 ){
             nickNameSet[trans[i][1]] = trans[i][2];
        }
    }
    
    for(var i=0; i<trans.length; i++){
        if(trans[i][0]==='Enter'){
            answer.push(nickNameSet[trans[i][1]]+'님이 들어왔습니다.');
        } else if(trans[i][0]==='Leave'){
            answer.push(nickNameSet[trans[i][1]]+'님이 나갔습니다.');
        }
      }
