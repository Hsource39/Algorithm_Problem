...
/*
다음부턴 처음 틀린 풀이도 잘 저장해놓고 문제를 풀게 된 과정을 적어야 겠다.
...
키는 사람이름이고 값은 추천인과 번 돈을 키를 가진 객체를 만들게 되고

판매를 시작하면
먼저 판매자 객체를 지정한 뒤 판매를 통해 얻은 돈을 구해주고
판매자의 referral이 없을 때 '-': { money: 0 } <- 이 때 반복문을 멈추게 한 뒤
돈이 0일 경우는 바로 반복문을 멈추고
그렇지 않을 경우 줘야할 돈을 뺀 나머지를 obj.money 에 넣어주고 변수 money에는 줘야할 돈을 넣어준 뒤 obj를 계속해서 바꿔주게 된다.
...
이렇게 돈이 나뉘게 되고
이 객체의 돈을 전부 answer에 넣어준 뒤 '-'에 해당하는 0번째 인덱스를 삭제해서 리턴해준다.
*/
